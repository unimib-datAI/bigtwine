...
parser.add_argument("working_dir", help="Path to the working directory. It must be writable.")
...
print >> sys.stderr, "Working dir: %s" % options.working_dir
...
if options.working_dir is None or options.working_dir == "":
    print >> sys.stderr, "No working dir given."
...
print >> sys.stderr, "Finished loading all models. Now waiting for files in %s and writing to %s"  % (INPUT_DIR, OUTPUT_DIR)
...
def process_document(input_file, output_file):
    ...
    print >> sys.stderr, "Start reading from %s and writing to %s" % (input_file, output_file)
    ...
    with open(input_file) as fp:
        ...
        while nLines == 0 or len(line) > 1:
            ...
            if len(row) > 1:
                ...
                line = ''
            if not line:
                print >> sys.stderr, "Finished reading %s lines from %s" % (nLines -1, input_file)
                break
            ...
            for i in range(len(features.entities)):
                ...
                if llda and len(wids) > 0:
                    ...
                    for label in count.keys():
                        ...
                        print >> out_fp,"[#ETS#]\t",features.entityStrings[i].encode('utf-8'),"\t",label,"\t",p
...
def make_dir_structure(working_dir):
    dirs = [INPUT_DIR, PROCESSING_DIR, OUTPUT_DIR]
    for dir_path in dirs:
        ...
        try:
            ...
            print >> sys.stderr, "Error raised on creating dir '{0}': {1}".format(dir_path, err)
